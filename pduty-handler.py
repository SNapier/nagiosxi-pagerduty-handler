import requests, sys, argparse, os, logging, json, time
...
def pdEventType(meta):
    ...
    #DEBUG OUT
    if meta.debug:
        print(cName+"-DEBUG: pid["+str(os.getpid())+"] stateTypes["+i+"] eventType["+i+"]")
...
def getDedupeKey(i, meta):
    ...
    #DEBUG OUT
    if meta.debug:
        print(cName+"-DEBUG: pid["+str(os.getpid())+"] eventType["+i+"] dedupKey["+str(switcher.get(i))+"]")
...
def payloadManifest(etype,dedupe_key,meta):
    ...
    #DEBUG OUT
    if meta.debug:
        print(cName+"-DEBUG: pid["+str(os.getpid())+"] eventType["+etype+"] payload["+payload+"]")
...
def sendPagerDutyEvent(meta, payload):
    ...
    #POST INCIDENT
    r = requests.post(url = apiEndpoint, data=payload)
    ...
    #FIRST TRY FAILED WITH A BUSY SIGNAL
    if r.status_code == 429:
        
        #THROW WARNING FAILED FOR FIRST ATEMPT
        #TODO CONVERT TO LOG OUT
        print(cName+"-WARNING: pid["+str(os.getpid())+"] eventType["+etype+"] payload["+payload+"] sendEventAttempt["+str(attempt)+"] statusCode["+str(r.status_code)+"] Sleeping for "+str(sleepDuration)+" before retrying.")    

        #DEBUG OUT
        if meta.debug:
             print(cName+"-DEBUG: pid["+str(os.getpid())+"] eventType["+etype+"] payload["+payload+"] sendEventAttempt["+str(attempt)+"] statusCode["+str(r.status_code)+"] Sleeping for "+str(sleepDuration)+" before retrying.")
        ...
        r2 = requests.post(url=apiEndpoint, params=payload)

        #SECOND ATTEMPT AT SEND EVENT
        if r2.statsu_code != 202:
            if meta.debug:
                print(cName+"-DEBUG: pid["+str(os.getpid())+"] eventType["+etype+"] statusCode["+str(r.status_code)+"] sendEventAttempt["+str(attempt)+"] Failed with status code "+str(r2.status_code))    
            #FAILED WE EXIT
            print(cName+"-ERROR: pid["+str(os.getpid())+"] eventType["+etype+"] statusCode["+str(r.status_code)+"] sendEventAttempt["+str(attempt)+"] Failed with status code "+str(r2.status_code))
            sys.exit()
        else:
            if meta.debug:
                print(cName+"-DEBUG: pid["+str(os.getpid())+"] eventType["+etype+"] statusCode["+str(r.status_code)+"] sendEventAttempt["+str(attempt)+"] Successfuly sent event to pager duty")
    ...
    #FIRST TRY WAS SUCCESSFUL        
        ...
        if meta.debug:
            print(cName+"-DEBUG: pid["+str(os.getpid())+"] eventType["+etype+"] statusCode["+str(r.status_code)+"] sendEventAttempt["+str(attempt)+"] Failed with status code "+str(r.status_code))
        
        #FAILED WE EXIT
        print(cName+"-ERROR: pid["+str(os.getpid())+"] eventType["+etype+"] statusCode["+str(r.status_code)+"] sendEventAttempt["+str(attempt)+"] Failed with message "+str(r.text))
        sys.exit()
...
def handleEvent(etype, meta):
    ...
    if etype == "resolve":
        ...
        print(cName+"-INFO: pid["+str(os.getpid())+"] Event was discarded, eventType["+etype+"]")
        sys.exit()

    else:
        #WE WERE GIVEN AN UNKNOWN EVENT TYPE WE EXIT
        print(cName+"-ERROR: pid["+str(os.getpid())+"] Failed to handle returned eventType. eventType["+etype+"] is unknown.")
        sys.exit()

    #DEBUG OUT
    if meta.debug:
        print(cName+"-DEBUG: pid["+str(os.getpid())+"] eventType["+etype+"] statusCode["+str(result.status_code)+"] result["+result.text+"]")    
...
if __name__ == "__main__" :
    ...
    event = argparse.ArgumentParser(prog=cName+"v:"+cVersion, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    if valid:
        ...
        if etype in triggerList:
            ...
            #LOG AND EXIT
            print(cName+"-INFO: pid["+str(os.getpid())+"] EventHandler received the \"discard\" event type.")
            sys.exit(2)
        else:
            ...
            print(cName+"-ERROR: pid["+str(os.getpid())+"] EventHandler Failed to determine the eventType from the handler input.")
            sys.exit()
    else:
        #WE SHOULD NOT BE HERE
        #HANDLER INPUT TO HELP WITH TROUBLESHOOTING.
        print(cName+"-CRITICAL: pid["+str(os.getpid())+"] EventHandler input failed validity judgement calls without exiting.")
        sys.exit()